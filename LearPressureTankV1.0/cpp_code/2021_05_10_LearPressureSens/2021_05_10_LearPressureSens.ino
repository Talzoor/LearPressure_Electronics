...
double pmin = -0.05; // minimum value of pressure range [bar, psi, kPa, etc.]
...
void loop() {
  ...
  /*
    ...
    Data format : Status Register, 24-bit Sensor Data, Digital Counts, percentage of full scale pressure, pressure output, temperature
  */
  //  sprintf(printBuffer, "%x\t%2x %2x %2x\t%s\t%s\t%s\t%s \n", data[0], data[1], data[2], data[3], cBuff, percBuff, pBuff, tBuff);
  //  sprintf(printBuffer, "%s\t%s%s\t%s \n", pBuff, percBuff, "%", tBuff);
