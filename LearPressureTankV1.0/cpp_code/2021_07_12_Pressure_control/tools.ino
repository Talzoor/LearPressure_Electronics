...
void read_default_vars() {
  ...
  for (int i = 0; i < 10; i++) {
    if (PressureSensor) {
      ReadVar_float(Pressure_target[i]);
      ReadVar_float(Pressure_step[i]);
      ReadVar_byte(Pressure_TimePerStep[i]);
      ReadVar_byte(Pressure_ProgRepeat[i]);
      ReadVar_byte(Pressure_NextProg[i]);
      ReadVar_byte(Pressure_ProgTrigOffset[i]);
    } else { // no PressureSensor
...
void decide(uint8_t _cur_screen, uint8_t cur_selection, uint8_t _button_click, uint8_t _button_event, uint8_t* _vars) {
  ...
  if (_button_click == BUTTON_ENC) {      // ------------- ENCODER -------------
    ...
    if (move_to_next_scr) {
      ...
      if (next_scr == 111)
      {
        ...
        else if (PressureSensor == 1) {
          next_scr = 112;
          set_scr112_vars(defaultProg);
        }
      }
      else if (next_scr == 11)
        set_scr11_vars();
  ...
  if (_button_click == BUTTON_UP) {       // ------------- BUTTON_UP -------------
    if (_cur_screen == 0) {
      ...
      if (strcmp_P(scr_cur_text, PSTR("Ps.trgt")) == 0) {
        float p_trg = Pressure_target[prog_num];
        p_trg += (inc * 0.01);
        p_trg = (p_trg > 2.01) ? 0.0 : p_trg;
        p_trg = constrain(p_trg, 0, 2); // fixes some bug with -0.00
        Pressure_target[prog_num] = p_trg;
      }
      if (strcmp_P(scr_cur_text, PSTR("Step")) == 0) {
        float p_step = Pressure_step[prog_num];
        p_step += (inc * 0.01);
        p_step = (p_step > 1.0) ? 0.01 : p_step; // need to be able to be on 0
        p_step = constrain(p_step, 0.005, 1); // fixes some bug with -0.00
        Pressure_step[prog_num] = p_step;
      }
      if (strcmp_P(scr_cur_text, PSTR("Time/stp")) == 0) {
        uint8_t time_per_step = Pressure_TimePerStep[prog_num];
        time_per_step += (inc * 1);
        time_per_step = (time_per_step > 59) ? 1 : time_per_step; // need to be able to be on 0
        Pressure_TimePerStep[prog_num] = time_per_step;
      }
      if (strcmp_P(scr_cur_text, PSTR("PrgOfst[s]")) == 0) {
        uint8_t prog_offset = Pressure_ProgTrigOffset[prog_num];
        prog_offset += (inc * 1);
        prog_offset = (prog_offset > 59) ? 1 : prog_offset; // need to be able to be on 0
        Pressure_ProgTrigOffset[prog_num] = prog_offset;
      }
      if (strcmp_P(scr_cur_text, PSTR("NextProg")) == 0) {
      ...
      if (strcmp_P(scr_cur_text, PSTR("Repeat")) == 0) {
        uint8_t prog_rpt = Pressure_ProgRepeat[prog_num];
        prog_rpt += (inc * 1);
        prog_rpt = (prog_rpt > 50) ? 0 : prog_rpt;
        Pressure_ProgRepeat[prog_num] = prog_rpt;
      }


      set_scr112_vars(prog_num);
      menu.update_screen();
    }
  ...
  if (_button_click == BUTTON_DOWN) {       // ------------- BUTTON_DOWN -------------
    if (_cur_screen == 0) {
      ...
      if (strcmp_P(scr_cur_text, PSTR("Ps.trgt")) == 0) {
        float p_trg = Pressure_target[prog_num];
        p_trg -= (inc * 0.01);
        p_trg = (p_trg < -0.01) ? 2.0 : p_trg; // need to be able to be on 0
        p_trg = constrain(p_trg, 0, 2); // fixes some bug with -0.00
        Pressure_target[prog_num] = p_trg;
      }
      if (strcmp_P(scr_cur_text, PSTR("Step")) == 0) {
        float p_step = Pressure_step[prog_num];
        p_step -= (inc * 0.01);
        p_step = (p_step < 0.01) ? 1.0 : p_step; // need to be able to be on 0
        p_step = constrain(p_step, 0.005, 1); // fixes some bug with -0.00
        Pressure_step[prog_num] = p_step;
      }
      if (strcmp_P(scr_cur_text, PSTR("Time/stp")) == 0) {
        uint8_t time_per_step = Pressure_TimePerStep[prog_num];
        time_per_step -= (inc * 1);
        time_per_step = (time_per_step < 1) ? 59 : time_per_step; // need to be able to be on 0
        Pressure_TimePerStep[prog_num] = time_per_step;
      }
      if (strcmp_P(scr_cur_text, PSTR("PrgOfst[s]")) == 0) {
        uint8_t prog_offset = Pressure_ProgTrigOffset[prog_num];
        prog_offset -= (inc * 1);
        prog_offset = (prog_offset < 1) ? 59 : prog_offset;
        Pressure_ProgTrigOffset[prog_num] = prog_offset;
      }
      if (strcmp_P(scr_cur_text, PSTR("NextProg")) == 0) {
      ...
      if (strcmp_P(scr_cur_text, PSTR("Repeat")) == 0) {
        int8_t prog_rpt = Pressure_ProgRepeat[prog_num];
        prog_rpt -= (inc * 1);
        prog_rpt = (prog_rpt < 0) ? 50 : prog_rpt;
        Pressure_ProgRepeat[prog_num] = prog_rpt;
      }
      set_scr112_vars(prog_num);
      menu.update_screen();
...
void set_scr112_vars(uint8_t _prog) {
  uint8_t n = 0;
  global_vars_init();
  var_default[n++] = _prog;
  var_default[n++] = 0; //pressure target replacment
  var_default[n++] = 0; //pressure step
  var_default[n++] = Pressure_TimePerStep[_prog];
  var_default[n++] = Pressure_ProgTrigOffset[_prog];
  var_default[n++] = Pressure_NextProg[_prog];
  var_default[n++] = Pressure_ProgRepeat[_prog];

  char bf[10];
  Debug.print(DBG_INFO, F("vars_scr_112="));
  Debug.timestampOff();
  Debug.print(DBG_INFO, F("[%s]:%d, "), "_prog", var_default[0]);
  Debug.print(DBG_INFO, F("[%s]:%s, "), "p_trg", dtostrf(Pressure_target[_prog], 2, 2, bf));
  Debug.print(DBG_INFO, F("[%s]:%s, "), "p_step", dtostrf(Pressure_step[_prog], 2, 2, bf));
  Debug.print(DBG_INFO, F("[%s]:%d, "), "TimePerStep", var_default[3]);
  Debug.print(DBG_INFO, F("[%s]:%d, "), "ProgTrigOffset", var_default[4]);
  Debug.print(DBG_INFO, F("[%s]:%d, "), "NextProg", var_default[5]);
  Debug.print(DBG_INFO, F("[%s]:%d\n"), "ProgRepeat", var_default[6]);

  Debug.timestampOn();

  menu.Set_vars(var_default);
  menu.Set_pressure_target_and_step(Pressure_target[_prog], Pressure_step[_prog]);
}
...
void save_vars_for_prog(uint8_t _prog_num) {
  if (PressureSensor) {
    WriteVar_float(Pressure_target[_prog_num]);
    WriteVar_float(Pressure_step[_prog_num]);
    WriteVar_byte(Pressure_TimePerStep[_prog_num]);
    WriteVar_byte(Pressure_ProgRepeat[_prog_num]);
    WriteVar_byte(Pressure_NextProg[_prog_num]);
    WriteVar_byte(Pressure_ProgTrigOffset[_prog_num]);
  } else { // no PressureSensor
...
void run_program_with_sensor_next_prog() {
  ...
  Debug.print(DBG_INFO, F("pressure_target:%s, current_pressure_sensor:%s, current_pressure_goal:%s, pressure_step_atm:%s\n"), \
              dtostrf(pressure_target, 2, 3, bf0), \
              dtostrf(current_pressure_sensor, 2, 3, bf1), \
              dtostrf(current_pressure_goal, 2, 3, bf2), \
              dtostrf(pressure_step_atm, 2, 3, bf3));
...
void stop_program_now(bool _stop_all) {
  ...
  if (!_stop_all) {   //next prog
    ...
    if (prog_to_run != 10) {
      next_p_timer = timer.in(PAUSE_BETWEEN_PROGS, run_program_now_no_sensor); // 10=stop, otherwise run next prog
...
void pulse_on_now() {
  ...
  char in_out_str[10];
  strcpy(in_out_str, (!prog_in_out) ? "IN" : "OUT");
  char buff[100];
  sprintf_P(buff, PSTR("ON ------------- (dir:%s)(pin%d)\n"), in_out_str, pin);
  Debug.print(DBG_INFO, buff);
  Log(LOG_LEVEL_INFO, buff, false);
...
void pulse_off_now() {
  ...
  strcpy(in_out_str, (!prog_in_out) ? "IN" : "OUT");
  char buff[100];
  sprintf_P(buff, PSTR("OFF ------------- (dir:%s)(pin%d)\n"), in_out_str, pin);
  Debug.print(DBG_INFO, buff);
  Log(LOG_LEVEL_INFO, buff, false);
...
void init_prog_vars(uint8_t _prog) {
  ...
  if (PressureSensor == 0) {
    ...
    prog_next = Pressure_NextProg[prog_to_run];
  }
}
