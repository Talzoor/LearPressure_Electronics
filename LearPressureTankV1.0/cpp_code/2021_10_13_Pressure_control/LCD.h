...
class lcd_menu
{
  public:
    ...
    bool print_left_dot() {
      bool tmp_b = true;
      if (this->cur_screen == 99 or \
          this->cur_screen == 0 or \
          this->cur_screen == 14 or \
          this->cur_screen == 90 or \
          this->cur_screen == 97 or \
          this->cur_screen == 80 or \
          this->cur_screen == 98)
        tmp_b = false;
      return tmp_b;
    }
    ...
    void print_screen(byte _scr, uint8_t *_var) {
      ...
      if (_var[n - 1] != NULL_VAL) {
        ...
        if (strcmp_P(_tmp_sec_str, PSTR("Fill")) == 0) {
          ...
          if (prog_to_run != 10) {
            char bf[10];
            sprintf_P(buffer, PSTR("%dx%d P=%s"), \
                      prog_to_run, \
                      pressure_repeat, dtostrf(this->pressure, 2, 2, bf)[1]); //[1] to start from .00
          ...
          char bf[10];
          dtostrf(this->pressure, 2, 3, bf);
          if (this->pressure < 0.99) mid(bf, 1, strlen(bf), bf, sizeof(bf));
          sprintf_P(buffer, PSTR("%s   ATM "), bf/*P_val*/); //to start from .00
      ...
      // ------------------- line 2 only -------------------
      if (_var[n] == NULL_VAL) { // without var
      ...
      if (_var[n] != NULL_VAL) {
        ...
        if (strcmp_P(_tmp_sec_str, PSTR("T[m:s]")) == 0) {
          ...
          char bf[10];
          dtostrf(this->pressure_target, 2, 3, bf);
          if (this->pressure_target < 0.99) mid(bf, 1, strlen(bf), bf, sizeof(bf));
          sprintf_P(buffer, PSTR(" %s "), bf/*P_val*/); //to start from .00
      ...
      if (r_screen_exist) {
        ...
        char *str = this->R_screen_line(R_scr_idx + 1); //first R line
        ...
        str = this->R_screen_line(R_scr_idx + 2); //sec R line
        if (strcmp_P(str, PSTR("Stop")) == false) {
          lcd.setCursor(16 - 1, 1);
          lcd.write(6);
          strcpy_P(str, empty_str);
        }
        str_len = strlen(str);
    ...
    void update_buffer(char* _buf) {
      char bf[10];
      dtostrf(this->pressure, 2, 3, bf);
      // if 0<p<1 :
      if (this->pressure < 0.999 and this->pressure > 0.001) mid(bf, 1, strlen(bf), bf, sizeof(bf));
      if (this->pressure == -1.0) {
        sprintf_P(_buf, PSTR("err "));
      } else {
        sprintf_P(_buf, PSTR("%s "), bf/*P_val*/); //to start from .00
      }
    }
...
//            Serial.println("p_buf:" + (String)p_buf);
...
void sd_init_msg(bool _init_val) {
  if (_init_val) { //ok
    menu.print_screen(98);
  } else { //not ok
    menu.print_screen(97);
    delay(5000);
  }
}
