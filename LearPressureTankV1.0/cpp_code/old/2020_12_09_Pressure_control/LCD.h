#ifdef LCD_ON
#define LCD_ROWS 16
//LiquidCrystal_I2C lcd(0x3F, LCD_ROWS, 2);
  ...
  public:
    ...
    void print_screen(byte _scr, uint8_t *_var) {
      ...
      if (_var[n - 1] != NULL_VAL) {
        ...
        else if (strcmp_P(_tmp_sec_str, PSTR("Runing P")) == 0) {
          if (prog_to_run != 10) {
            sprintf_P(buffer, PSTR("%d"), prog_to_run);
...
void main_screen() {
  ...
  menu.Reset_var();
  menu.Set_vars_by_i(0, defaultProg);
  menu.print_screen(0);
