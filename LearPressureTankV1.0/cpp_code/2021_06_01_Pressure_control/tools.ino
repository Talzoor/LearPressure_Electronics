...
void decide(uint8_t _cur_screen, uint8_t cur_selection, uint8_t _button_click, uint8_t _button_event, uint8_t* _vars) {
  ...
  if (_button_click == BUTTON_UP) {       // ------------- BUTTON_UP -------------
    if (_cur_screen == 0) {
      ...
      if (PressureSensor == 0) run_program_now_no_sensor();
      else run_program_now_with_sensor();
    } //scr=13
    else if (_cur_screen == 111) {
      ...
      if (strcmp_P(scr_cur_text, PSTR("Step")) == 0) {
        ...
        p_step += (inc * 0.05);
        p_step = (p_step > 1.0) ? 0.05 : p_step; // need to be able to be on 0
        p_step = constrain(p_step, 0.05, 1); // fixes some bug with -0.00
  ...
  if (_button_click == BUTTON_DOWN) {       // ------------- BUTTON_DOWN -------------
    if (_cur_screen == 0) {
      ...
      if (strcmp_P(scr_cur_text, PSTR("Step")) == 0) {
        float p_step = Pressure_step[prog_num];
        p_step -= (inc * 0.05);
        p_step = (p_step < 0.05) ? 1.0 : p_step; // need to be able to be on 0
        p_step = constrain(p_step, 0.05, 1); // fixes some bug with -0.00
        Pressure_step[prog_num] = p_step;
      }
      if (strcmp_P(scr_cur_text, PSTR("Time/stp")) == 0) {
